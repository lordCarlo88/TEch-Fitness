<template>
  <div>
    <h1>{{ $store.state.user.username }}'s Dinner History</h1>
    <div class="viewDin">
      ...
      />
      <br /><br />
      <button id="confirmDate" v-on:click="getViewDinner">
...
export default {
  ...
  methods: {
    getViewDinner() {
      ViewMealService.getViewDinner(this.$store.state.user.id, this.date)
        .then((response) => {
