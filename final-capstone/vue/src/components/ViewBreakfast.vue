<template>
  <div>
    <h1>{{ $store.state.user.username }}'s Breakfast History</h1>
    <div class="viewBreak">
      ...
      />
      <br /><br />
      <button id="confirmDate" v-on:click="getViewBreakfast">
...
export default {
  ...
  methods: {
    getViewBreakfast() {
      ViewMealService.getViewBreakfast(this.$store.state.user.id, this.date)
        .then((response) => {
