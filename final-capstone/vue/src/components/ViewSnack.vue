<template>
  <div>
    <h1>{{ $store.state.user.username }}'s Snack History</h1>
    <div class="viewSnackers">
      ...
      />
      <br /><br />
      <button id="confirmDate" v-on:click="getViewSnack">
...
export default {
  ...
  methods: {
    getViewSnack() {
      ViewMealService.getViewSnack(this.$store.state.user.id, this.date)
        .then((response) => {
